...
public class Graph {
	...
	// add an edge from one node to another; if this is a weighted graph this
	...
	public static void main(String[] args) {
		...
		System.out.println(graph.getShortestPath("A", "E")); // should print [A,
																// K, E]
	}
	...
	public List<String> getShortestPath(String from, String to) {
		...
		while (!queue.isEmpty()) {
			...
			if (node.equals(to)) {
				return buildPath(from, to, reversepath);
			}
			visited.add(node);
			visitNeighbors(node, visited, queue, reversepath);
		}
		return null;
	}

	public void visitNeighbors(String Node, Set<String> visited, List<String> queue, Map<String, String> reversePath) {
		List<Edge> neighbors = graphData.get(Node); //
		if (neighbors != null) {
			for (Edge edges : neighbors) {
				String neighbor = edges.adjacentNode;
				if (!visited.contains(neighbor)) {
					if (!reversePath.containsKey(neighbor)) {
						reversePath.put(neighbor, Node);
					}
					queue.add(neighbor);
				}
			}
	...
	private List<String> buildPath(String from, String to, Map<String, String> reversepath) {

		List<String> result = new LinkedList<String>();
		String current = to; 
		result.add(to);
		while (!current.equals(from)) {
			current = reversepath.get(current);
